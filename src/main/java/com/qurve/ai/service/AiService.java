...
public class AiService {
    ...
    /**
     * 현재 학습 언어의 AI 학습 도우미 채팅
     *
     * * 사용자의 현재 학습 언어에 맞는 AI 프롬프트를 적용한다.
     * * 해당 언어 채팅방의 최근 20개 메시지를 문맥으로 포함한다.
     * * 사용자와 AI의 대화 내용은 언어별 채팅방에 저장된다.
     *
    ...
    public AiChatResponseDto chat(String loginId, AiChatRequestDto requestDto) {
        ...
        // 언어와 관계없이 AI 채팅을 한 번도 사용하지 않았는지 확인
        boolean firstAiUse = !aiChatRoomRepository.existsByUser(user);

        // 현재 학습 언어의 채팅방 조회 또는 생성
        AiChatRoom room = aiChatRoomRepository
                .findByUserAndLearningLanguage(user, learningLanguage)
                .orElseGet(() -> aiChatRoomRepository.save(
                        AiChatRoom.builder()
                                .user(user)
        ...
        boolean usedAiToday =
                xpHistoryRepository.existsByUserAndActionTypeAndEarnedAtBetween(
                        user,
        ...
        // 현재 학습 언어 채팅방의 최근 메시지만 조회
        List<AiChatMessage> recentMessages = aiChatMessageRepository.findTop20ByRoomOrderByCreatedAtDesc(room);

        String personalizedInfo = buildPersonalizedInfo(user);

        String aiResponse = callGemini(
    ...
    private String callGemini(String userMessage, List<AiChatMessage> recentMessages, String personalizedInfo, LearningLanguage learningLanguage) {
        ...
        String systemPrompt = buildSystemPrompt(learningLanguage, personalizedInfo);
    ...
    private String buildSystemPrompt(LearningLanguage learningLanguage, String personalizedInfo) {
        ...
        return switch (learningLanguage) {
            case JAPANESE -> buildJapaneseSystemPrompt(personalizedInfo);
            case ENGLISH -> buildEnglishSystemPrompt(personalizedInfo);
        };
    }

    private String buildJapaneseSystemPrompt(String personalizedInfo) {
        return """
    ...
    private String buildEnglishSystemPrompt(String personalizedInfo) {

        return """
            당신은 영어 학습 앱 QURVE의 영어 전문 AI 튜터이자 개인 학습 코치입니다.
            ...
            [역할]
            다음 범위에 해당하는 질문에 답변하세요.
            - 영어 단어, 문법, 발음, 듣기, 독해 및 표현
            - TOEIC 학습법, 문제 풀이 및 시험 정보
            - 오답 원인 분석과 정답 해설
            - 정답률과 학습 패턴 분석
            - 사용자 학습 데이터를 활용한 피드백과 학습 방향 제안
            - 영어 학습과 자연스럽게 연결되는 후속 질문

            위 범위와 관련 없는 질문에는 다른 내용을 덧붙이지 말고 다음 문장으로만 답변하세요.
            "죄송합니다. 저는 영어 학습 관련 질문만 답변할 수 있습니다."

            [답변 원칙]
            - 설명은 기본적으로 한국어로 제공하세요.
            - 친근하고 격려하는 학습 코치의 말투를 유지하세요.
            - 질문 성격에 따라 답변 길이를 조절하세요.
            - 단순한 단어 질문에는 핵심을 짧게 답변하세요.
            - 불필요한 인사, 맺음말, 구분선과 반복적인 추가 질문을 사용하지 마세요.
            - 문법, 문제 풀이, 오답 분석에는 이해에 필요한 과정을 단계적으로 설명하세요.
            - 단어를 설명할 때는 철자, 발음, 품사와 의미를 구분하세요.
            - 필요한 경우 자연스러운 영어 예문과 한국어 뜻을 제공하세요.
            - 영어 예문의 철자와 문법이 정확한지 확인하세요.
            - 사용자의 잘못된 전제나 표현에 무조건 동의하지 말고 정확하게 교정하세요.
            - 같은 설명을 불필요하게 반복하지 마세요.
            ...
            - 답변마다 데이터 출처나 개인화 근거를 임의로 언급하지 마세요.
            - 사용자가 추천 이유나 분석 근거를 물어보면 활용한 데이터를 간단히 설명하세요.
            - 학습 기록이 없거나 부족하면 일반적인 학습 방법을 안내하세요.

            [정확성]
            - 확실하지 않은 내용은 사실처럼 단정하지 마세요.
            - 최신 TOEIC 일정, 접수 기간, 시험장 등 변동 가능한 정보는 추측하지 말고 공식 TOEIC 채널에서 확인하도록 안내하세요.
            - 예문은 학습자의 현재 수준을 고려해 자연스럽고 이해하기 쉽게 작성하세요.
     ...
     * 현재 학습 언어의 대화 초기화
     *
     * * 현재 학습 언어 채팅방의 메시지만 삭제하고 채팅방은 유지한다.
     * * 다른 언어의 대화와 학습 데이터(오답, XP 등)는 삭제되지 않는다.
     *
     * @param loginId 로그인 ID
     * @throws BusinessException 유저가 존재하지 않는 경우
     */
