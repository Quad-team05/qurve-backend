...
public class ChallengeProgressService {
    ...
    /**
     * 현재 학습 언어의 챌린지 진행도 반영
     *
     * * 사용자의 현재 학습 언어를 기준으로 챌린지 진행도를 반영한다.
     * * 활동 언어가 별도로 정해진 호출에서는 언어를 전달하는 메서드를 사용한다.
     *
     ...
     * 활동 언어별 챌린지 진행도 반영
     *
     * * 활동 언어와 목표 유형이 일치하는 활성 챌린지에 진행도를 반영한다.
     * * KST 기준 오늘이 챌린지 기간에 포함되는 경우에만 반영한다.
     * * 활동 언어와 기존 챌린지의 언어가 null이면 일본어로 처리한다.
     *
     * @param user 활동을 수행한 사용자
     * @param learningLanguage 활동이 발생한 학습 언어
     * @param goalType 누적할 챌린지 목표 유형
     * @param amount 증가시킬 달성값
     */
    @Transactional
    public void addProgress(User user, LearningLanguage learningLanguage, ChallengeGoalType goalType, int amount) {
        ...
        LearningLanguage language = learningLanguage == null ? LearningLanguage.JAPANESE : learningLanguage;
